...
@Component({
  ...
  template: `
    <nav
      class="fixed top-0 left-0 right-0 z-50 bg-[var(--secondary-bg)] border-b border-[var(--border-color)]"
    >
      <div class="px-6 h-16 flex items-center justify-between">
        <a routerLink="/chat" class="no-underline">
          <h1
            class="text-2xl font-semibold text-[var(--text-primary)] hover:text-[var(--text-primary)] transition-colors"
          >
            Quincy
          </h1>
        </a>
