...
describe('addMessage', () => {
  ...
  let mockScrollToBottomRequest: any;
  ...
  describe('基本機能', () => {
    it('ユーザーメッセージを正しく追加する', () => {
      const messageId = addMessage('Hello', 'user', mockAppStore, mockScrollToBottomRequest);
  ...
  describe('セッション状態の処理', () => {
    it('セッションがnullの場合でもメッセージを追加する', () => {
      ...
      const messageId = addMessage('Message', 'user', mockAppStore, mockScrollToBottomRequest);
  ...
  describe('メッセージIDの生成', () => {
    it('一意のメッセージIDを生成する', () => {
      const messageId1 = addMessage('Message 1', 'user', mockAppStore, mockScrollToBottomRequest);
      const messageId2 = addMessage('Message 2', 'user', mockAppStore, mockScrollToBottomRequest);
  ...
  describe('タイムスタンプの処理', () => {
    it('現在時刻のタイムスタンプを設定する', () => {
      ...
      addMessage('Message', 'user', mockAppStore, mockScrollToBottomRequest);
      const afterTime = new Date();

      const callArgs = mockAppStore.addChatMessage.mock.calls[0][0];
  ...
  describe('エッジケース', () => {
    it('空のコンテンツでもメッセージを追加する', () => {
      const messageId = addMessage('', 'user', mockAppStore, mockScrollToBottomRequest);
    ...
    it('非常に長いコンテンツでもメッセージを追加する', () => {
      ...
      const messageId = addMessage(longContent, 'user', mockAppStore, mockScrollToBottomRequest);
