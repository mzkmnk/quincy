...
@Component({
  ...
  template: `
    <div
      class="border-b border-[var(--border-color)] p-4 bg-[var(--secondary-bg)] sticky top-0 z-90"
    >
      <div class="flex items-center justify-between">
        <div>
          @if (appStore.currentQSession()) {
            <h1 class="text-xl font-semibold text-[var(--text-primary)]">
              {{ getProjectName(appStore.currentQSession()!.projectPath) }}
            </h1>
          } @else if (appStore.currentQConversation()) {
            <h1 class="text-xl font-semibold text-[var(--text-primary)]">
              {{ getProjectName(getProjectPathFromConversation()) }}
            </h1>
            ...
            <p class="text-sm text-[var(--text-secondary)] mt-1">
              Please wait while we start your session
            </p>
...
export class ChatHeaderComponent {
  ...
  getProjectPathFromConversation(): string {
    ...
    const historyItem = this.appStore
      .amazonQHistory()
      .find(h => h.conversation_id === currentConversation.conversation_id);
