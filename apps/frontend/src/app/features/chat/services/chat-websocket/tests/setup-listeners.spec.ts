...
describe('setupChatWebSocketListeners', () => {
  ...
  describe('複数回の呼び出し', () => {
    ...
    it('異なるハンドラーで複数回呼び出せる', () => {
      ...
      expect(mockWebSocketService.setupChatListeners).toHaveBeenNthCalledWith(1,
        handlers1.onQResponse, handlers1.onQError, handlers1.onQInfo, handlers1.onQCompletion
      );
      expect(mockWebSocketService.setupChatListeners).toHaveBeenNthCalledWith(2,
        handlers2.onQResponse, handlers2.onQError, handlers2.onQInfo, handlers2.onQCompletion
      );
    });
  });
  ...
  describe('実際の使用シナリオ', () => {
    ...
    it('ハンドラー内でエラーが発生してもsetup関数自体は正常終了する', () => {
      const throwingHandlers: ChatWebSocketHandlers = {
        onQResponse: () => { throw new Error('Handler error'); },
