  ...
  try {
    if (!qHistoryService.isDatabaseAvailable()) {
      sendErrorCallback(
        ...
        'Amazon Q database is not available. Please ensure Amazon Q CLI is installed and has been used at least once.'
      );
      return;
    }
    ...
    if (errorMessage.includes('データベースにアクセスできません')) {
      ...
      userFriendlyMessage =
        'Amazon Q database file not found. Please use Amazon Q CLI at least once to create the database.';
      ...
      userFriendlyMessage =
        'Permission denied accessing Amazon Q database. Please check file permissions.';
    }
