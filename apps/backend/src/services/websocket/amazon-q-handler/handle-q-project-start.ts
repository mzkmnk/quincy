...
export async function handleQProjectStart(
  ...
  sendErrorCallback: (socket: Socket<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>, code: string, message: string, details?: Record<string, string | number | boolean | null>) => void
): Promise<void> {
  try {
    // Amazon Q CLIの可用性をまずチェック
    const cliCheck = await qCliService.checkCLIAvailability();
    if (!cliCheck.available) {
      sendErrorCallback(socket, 'Q_CLI_NOT_AVAILABLE', 
