...
describe('Performance Tests', () => {
  ...
  describe('ANSI除去パフォーマンス', () => {
    ...
    it('長いテキストのANSI除去が適切な時間で実行されること', () => {
      const longText = '\x1b[31m' + 'A'.repeat(10000) + '\x1b[0m \x1b[32m' + 'B'.repeat(10000) + '\x1b[0m';
      const startTime = performance.now();
  ...
  describe('サービス初期化パフォーマンス', () => {
    it('Amazon Q CLIサービスの初期化が高速に実行されること', () => {
      ...
      console.log(`Initialized Amazon Q CLI service ${iterations} times in ${duration.toFixed(2)}ms`);
  ...
  describe('エラーハンドリングパフォーマンス', () => {
    it('大量のエラーが発生しても処理が継続できること', async () => {
      ...
      console.log(`Handled ${errorCount} errors and ${successCount} successes in ${duration.toFixed(2)}ms`);
